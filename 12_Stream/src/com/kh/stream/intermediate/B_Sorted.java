...
public class B_Sorted {
	...
	public void method1() {
		...
		Arrays.stream(new int[] {3, 5, 1, 2, 4})
		.boxed()
		.sorted(Comparator.reverseOrder()).forEach(value -> System.out.print(value + " "));
	}
		//요소가 객체일 때
	public void method2() {
			...
			System.out.println();
			//Comparable 인터페이스 구현 내용에 따라서 반대로 정렬(내림차순 정렬)
			names.stream().sorted(Comparator.reverseOrder()).forEach(name -> System.out.print(name + " ")); //문자는 박스 안써도 됨
