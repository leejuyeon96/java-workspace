 ...
 * -filter(Predicate) : 매개 값으로 전달되는 Predicate 가 false를 리턴하는 요소를 제거
 */
...
public class A_Filtering {
	...
	List<Student> students = Arrays.asList(new Student("김준영", 8, "남자", 80, 50), new Student("김도경", 6, "여자", 80, 50),
	...
	//수학점수 70이상 영어점수 80이상 요소만
		students.stream().distinct().filter(student -> student.getMath() >= 70 && student.getEnglish() >= 80).forEach
		(System.out::println);
	}
