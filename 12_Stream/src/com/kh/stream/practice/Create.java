 ...
 * 
 * 스트림의 종류
 * -java.util.stream 패키지에 존재하고 BaseStream 인터페이스를 자식인터페이스들이 상속관계를 이루고있다.
 * 
 * Stream API
 ...
 * -중간처리와 최종처리 메서드를 구분하는 방법
 *  : 리턴타입으로 알 수 있음
 *    리턴타입이 Stream 이라면 중간 처리 메서드 
 *    리턴타입이 기본타입이거나 Optional 객체라면 최종 처리 메서드
 *    
...
public class Create {
	...
	//숫자 범위로 스트림을 생성하는 방법
	//IntStream, LongStream, DoubleStream 의 range(), rangeClosed() 메서드를 이용해서 생성
	
	public void method1() {
		//첫번째 매개값 ~ 두번째 매개값 이전까지의 값을 요소로 가지는 스트림 객체를 생성 
